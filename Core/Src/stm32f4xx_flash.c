...
|---------------------------------------------------------------------| | |
voltage range  | voltage range  | voltage range   | voltage range   | | | 2.7 V
- 3.6 V  | 2.4 V - 2.7 V  | 2.1 V - 2.4 V   | 1.8 V - 2.1 V   |
 |---------------|----------------|----------------|-----------------|-----------------|
...
<= 100 |
 |---------------|----------------|----------------|-----------------|-----------------|
 |5WS(6CPU cycle)|150< HCLK <= 168|120< HCLK <= 144|110 < HCLK <= 132|100 < HCLK
<= 120|
 |---------------|----------------|----------------|-----------------|-----------------|
...
<= 100 |
 |---------------|----------------|----------------|-----------------|-----------------|
 |5WS(6CPU cycle)|120< HCLK <= 180|120< HCLK <= 144|110 < HCLK <= 132|100 < HCLK
<= 120|
 |---------------|----------------|----------------|-----------------|-----------------|
 |6WS(7CPU cycle)|      NA        |144< HCLK <= 168|132 < HCLK <= 154|120 < HCLK
<= 140|
 |---------------|----------------|----------------|-----------------|-----------------|
 |7WS(8CPU cycle)|      NA        |168< HCLK <= 180|154 < HCLK <= 176|140 < HCLK
<= 160|
 |---------------|----------------|----------------|-----------------|-----------------|
 |8WS(9CPU cycle)|      NA        |      NA        |176 < HCLK <= 180|160 < HCLK
<= 168|
 +-------------------------------------------------------------------------------------+
...
<= 40  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |2WS(3CPU cycle)|60 < HCLK <= 84 |48 < HCLK <= 72 |44 < HCLK <= 66  |40 < HCLK
<= 60  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |3WS(4CPU cycle)|      NA        |72 < HCLK <= 84 |66 < HCLK <= 84  |60 < HCLK
<= 80  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |4WS(5CPU cycle)|      NA        |      NA        |      NA         |80 < HCLK
<= 84  |
 +-------------------------------------------------------------------------------------+
...
<= 16   |
 |---------------|----------------|----------------|-----------------|-----------------|
 |1WS(2CPU cycle)|30 < HCLK <= 64 |24 < HCLK <= 48 |18 < HCLK <= 36  |16 < HCLK
<= 32  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |2WS(3CPU cycle)|64 < HCLK <= 90 |48 < HCLK <= 72 |36 < HCLK <= 54  |32 < HCLK
<= 48  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |3WS(4CPU cycle)|90 < HCLK <= 100|72 < HCLK <= 96 |54 < HCLK <= 72  |48 < HCLK
<= 64  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |4WS(5CPU cycle)|      NA        |96 < HCLK <= 100|72 < HCLK <= 90  |64 < HCLK
<= 80  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |5WS(6CPU cycle)|      NA        |       NA       |90 < HCLK <= 100 |80 < HCLK
<= 96  |
 |---------------|----------------|----------------|-----------------|-----------------|
 |6WS(7CPU cycle)|      NA        |       NA       |        NA       |96 < HCLK
<= 100 |
 +-------------------------------------------------------------------------------------+
...
range   | voltage range 2.7 V - 3.6 V | |               | 2.7 V - 3.6 V  | 2.4 V
- 2.7 V  | 2.1 V - 2.4 V   | 1.8 V - 2.1 V   | with External Vpp = 9V      |
 |---------------|----------------|----------------|-----------------|-----------------|-----------------------------|
...
|           11                |
 +-------------------------------------------------------------------------------------------------------------------+
      ...
      -@- On STM32F405xx/407xx and STM32F415xx/417xx devices:
           (++) when VOS = '0' Scale 2 mode, the maximum value of fHCLK =
144MHz.
           (++) when VOS = '1' Scale 1 mode, the maximum value of fHCLK =
168MHz.
          [..]
          On STM32F42xxx/43xxx devices:
           (++) when VOS[1:0] = '0x01' Scale 3 mode, the maximum value of fHCLK
is 120MHz.
           (++) when VOS[1:0] = '0x10' Scale 2 mode, the maximum value of fHCLK
is 144MHz if OverDrive OFF and 168MHz if OverDrive ON.
           (++) when VOS[1:0] = '0x11' Scale 1 mode, the maximum value of fHCLK
is 168MHz if OverDrive OFF and 180MHz if OverDrive ON.
          [..]
 ...
 *
 * @note For STM32F405xx/407xx, STM32F415xx/417xx, STM32F401xx/411xE and
 * STM32F412xG devices this parameter can be a value between FLASH_Latency_0 and
 * FLASH_Latency_7.
 *
...
VoltageRange)
      ...
      (+) FLASH_Status FLASH_ProgramByte(uint32_t Address, uint8_t Data)
          The following functions can be used only for STM32F42xxx/43xxx
devices.
...
access (recommended to protect the FLASH memory against possible unwanted
operation)
 ...
 *
 *  @note  For STM32F405xx/407xx and STM32F415xx/417xx devices this parameter
 * can be a value between FLASH_Sector_0 and FLASH_Sector_11.
 *
 ...
 *
 *         For STM32F411xE and STM32F412xG devices this parameter can be a value
 * between FLASH_Sector_0 and FLASH_Sector_7.
 *
 ...
 *            @arg VoltageRange_1: when the device voltage range is 1.8V
 * to 2.1V, the operation will be done by byte (8-bit)
 *            @arg VoltageRange_2: when the device voltage range is 2.1V
 * to 2.7V, the operation will be done by half word (16-bit)
 *            @arg VoltageRange_3: when the device voltage range is 2.7V
 * to 3.6V, the operation will be done by word (32-bit)
 *            @arg VoltageRange_4: when the device voltage range is 2.7V to 3.6V
 * + External Vpp, the operation will be done by double word (64-bit)
 *
 * @retval FLASH Status: The returned value can be: FLASH_BUSY2,
 * FLASH_ERROR_PROGRAM2, FLASH_ERROR_WRP2, FLASH_ERROR_OPERATION2 or
 * FLASH_COMPLETE2.
 */
 ...
 *
 * @note   This function must be used when the device voltage range is from 2.7V
 * to 3.6V.
 *
 * @note   If an erase and a program operations are requested simultaneously,
 *         the erase operation is performed before the program one.
 ...
 * @note   This function must be used when the device voltage range is from 2.1V
 * to 3.6V.
 *
 * @note   If an erase and a program operations are requested simultaneously,
 *         the erase operation is performed before the program one.
...
@verbatim
 ...
 ===============================================================================
    [..]
      ...
      (+) void FLASH_OB_PCROP1Config(uint32_t OB_PCROP, FunctionalState
NewState)
...
/**
 * @brief  Enables or disables the write protection of the desired sectors, for
 * the first 1 Mb of the Flash
 *
 ...
 *
 * @note   This function can be used only for STM32F42xxx/43xxx and
 * STM32F401xx/411xE devices.
 *
 * @note   After PCROP activation, Option Byte modification is not possible.
 *         Exception made for the global Read Out Protection modification level
 * (level1 to level0)
 ...
 *         The active value of nWRPi bits is inverted when PCROP mode is active,
 * this means if SPRMOD = 1 and WRPi = 1 (default value), then the user sector i
 * is read/write protected. In order to avoid activation of PCROP Mode for
 * undesired sectors, please follow the below safety sequence :
 *         - Disable PCROP for all Sectors using
 * FLASH_OB_PCROPConfig(OB_PCROP_Sector_All, DISABLE) function for Bank1 or
 * FLASH_OB_PCROP1Config(OB_PCROP_Sector_All, DISABLE) function for Bank2
 *         - Enable PCROP for the desired Sector i using
 * FLASH_OB_PCROPConfig(Sector i, ENABLE) function
 *         - Activate the PCROP Mode FLASH_OB_PCROPSelectionConfig() function.
 *
 * @param  OB_PCROP:  Select the Protection Mode of nWPRi bits
 *          This parameter can be one of the following values:
 *            @arg OB_PcROP_Disable: nWRPi control the write protection of
 * respective user sectors.
 *            @arg OB_PcROP_Enable: nWRPi control the  read&write protection
 * (PCROP) of respective user sectors.
 * @retval None
 */
 ...
 *
 * @note   This function can be used only for STM32F42xxx/43xxx ,
 * STM32F401xx/411xE and STM32F412xG devices.
 *
 * @param  OB_PCROP: specifies the sector(s) to be read/write protected or
 * unprotected. This parameter can be one of the following values:
 *            @arg OB_PCROP: A value between OB_PCROP_Sector0 and
 * OB_PCROP_Sector11 for STM32F42xxx/43xxx devices and between OB_PCROP_Sector0
 * and OB_PCROP_Sector5 for STM32F401xx/411xE devices.
 ...
 * @param  OB_PCROP: specifies the sector(s) to be read/write protected or
 * unprotected. This parameter can be one of the following values:
 *            @arg OB_PCROP: A value between OB_PCROP_Sector12 and
 * OB_PCROP_Sector23
 *            @arg OB_PCROP_Sector_All
 * @param  Newstate: new state of the Write Protection.
 *          This parameter can be: ENABLE or DISABLE.
 ...
 *
 * @note   This function can be used only for STM32F42xxx/43xxx devices and
 * STM32F401xx/411xE devices.
 *
 * @param  None
 ...
 *            @arg FLASH_FLAG_RDERR: FLASH (PCROP) Read Protection error flag
 * (STM32F42xx/43xxx and STM32F401xx/411xE devices)
 ...
 *            @arg FLASH_FLAG_PGPERR: FLASH Programming Parallelism error flag
 *            @arg FLASH_FLAG_PGSERR: FLASH Programming Sequence error flag
 *            @arg FLASH_FLAG_RDERR: FLASH Read Protection error flag
 * (STM32F42xx/43xxx and STM32F401xx/411xE devices)
 * @retval None
 */
 ...
 * @param  None
 * @retval FLASH Status: The returned value can be: FLASH_BUSY2,
 * FLASH_ERROR_PROGRAM2, FLASH_ERROR_WRP2, FLASH_ERROR_RD2,
 * FLASH_ERROR_OPERATION2 or FLASH_COMPLETE2.
 */
