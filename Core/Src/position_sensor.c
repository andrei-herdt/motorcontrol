...
void ps_warmup(EncoderStruct* encoder, int n) {
  /* Hall position sensors noisy on startup.  Take a bunch of samples to clear
   * this data */
  for (int i = 0; i < n; i++) {
    encoder->spi_tx_word = 0x0000;
    HAL_GPIO_WritePin(ENC_CS, GPIO_PIN_RESET);  // CS low
    HAL_SPI_TransmitReceive(&ENC_SPI, (uint8_t*)encoder->spi_tx_buff,
                            (uint8_t*)encoder->spi_rx_buff, 1, 100);
...
void ps_sample(EncoderStruct* encoder, float dt) {
  ...
  // for(int i = N_POS_SAMPLES-1; i>0; i--){encoder->count_buff[i] =
  // encoder->count_buff[i-1];} memmove(&encoder->angle_multiturn[1],
  // &encoder->angle_multiturn[0], (N_POS_SAMPLES-1)*sizeof(float)); // this is
  // much slower for some reason

  /* SPI read/write */
  encoder->spi_tx_word = ENC_READ_WORD;
  HAL_GPIO_WritePin(ENC_CS, GPIO_PIN_RESET);  // CS low
  HAL_SPI_TransmitReceive(&ENC_SPI, (uint8_t*)encoder->spi_tx_buff, (uint8_t*)encoder->spi_rx_buff,
                          1, 100);
  while (ENC_SPI.State == HAL_SPI_STATE_BUSY)
    ;                                       // wait for transmission complete
  HAL_GPIO_WritePin(ENC_CS, GPIO_PIN_SET);  // CS high
  ...
  int off_interp = off_1 + ((off_2 - off_1) * (encoder->raw - ((encoder->raw >> 9) << 9)) >>
                            9);  // Interpolate between lookup table entries
  ...
  // TWO_PI_F*fmodf(((float)(encoder->count-M_ZERO))/((float)ENC_CPR), 1.0f);
  encoder->angle_singleturn = encoder->angle_singleturn < 0 ? encoder->angle_singleturn + TWO_PI_F :
                                                              encoder->angle_singleturn;
  ...
  // encoder->elec_angle =
  // TWO_PI_F*fmodf((encoder->ppairs*(float)(encoder->count-E_ZERO))/((float)ENC_CPR),
  // 1.0f);
  encoder->elec_angle = encoder->elec_angle < 0 ?
                            encoder->elec_angle + TWO_PI_F :
                            encoder->elec_angle;  // Add 2*pi to negative numbers
  /* Rollover */
  int rollover = 0;
...
*/
  ...
  encoder->velocity = (encoder->angle_multiturn[0] - encoder->angle_multiturn[N_POS_SAMPLES - 1]) /
