...
void order_phases(EncoderStruct* encoder, ControllerStruct* controller, CalStruct* cal,
                  int loop_count) {
...
void calibrate_encoder(EncoderStruct* encoder, ControllerStruct* controller, CalStruct* cal,
  ...
  if (cal->time < T1) {
    ...
    // rotate voltage vector through one mechanical rotation in the positive
    // direction
    ...
    // rotate voltage vector through one mechanical rotation in the negative
    // direction
    cal->theta_ref -= W_CAL * DT;  //(cal->time-T1);
    controller->i_d_des = I_CAL;
    controller->i_q_des = 0.0f;
  ...
  for (int i = 0; i < N_LUT; i++) {
    ...
    for (int j = (-window) / 2; j < (window) / 2; j++) {
      ...
      if (index < 0) {
        index += (SAMPLES_PER_PPAIR * PPAIRS);
      } else if (index > (SAMPLES_PER_PPAIR * PPAIRS - 1)) {
        index -= (SAMPLES_PER_PPAIR * PPAIRS);
      }
...
void measure_lr(EncoderStruct* encoder, ControllerStruct* controller, CalStruct* cal,
                int loop_count) {
}
