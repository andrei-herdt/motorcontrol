...
typedef struct {
  ...
  float dtheta_mech, dtheta_elec,
      dtheta_elec_filt;                // Rotor mechanical and electrical angular velocit
  ...
  float i_scale, k_d, k_q, ki_d, ki_q, ki_fw,
      alpha;           // Current loop gains, current reference filter coefficient
  float d_int, q_int;  // Current error integrals
  int adc_a_offset, adc_b_offset, adc_c_offset,
      adc_vbus_offset;                                 // ADC offsets
  ...
  int phase_order;
  union {
    float commands[5];  // Making this easier to pass around without
                        // including foc.h everywhere
    struct {
      float p_des, v_des, kp, kd,
          t_ff;  // Desired position, velocity, gains, torque
    };
  };
  float v_max, v_ref,
      fw_int;               // output voltage magnitude, field-weakening integral
  ...
  uint8_t adc_pwm_divider;
  uint8_t num_pwms_passed;
} ControllerStruct;
